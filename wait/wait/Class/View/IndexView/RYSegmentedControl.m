...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles frame:(CGRect)frame delegate:(id)vc{
    ...
    if (self) {
        ...
        [self addScrollView:CGRectMake(0, 0, SCREENWIDTH, self.frame.size.height - 45) :vc];
        [self addTableView:CGRectMake(0, 0, SCREENWIDTH, self.scrollView.frame.size.height) :vc];
    }
    return self;
}
...
- (void)addTableView:(CGRect)frame :(id)vc{
    self.firstTableView = [[RYTableView alloc]initWithFrame:frame];
